...
docker run -d  --link netuitive-agent-api-test --name netuitive-agent-test -e DOCKER_HOSTNAME=testserver -e APIKEY=testapikey -e HTTPVAR=http -e LOGLEVEL=DEBUG -e APIHOST=$(docker inspect netuitive-agent-api-test | grep IPAddress | cut -d '"' -f 4|tail -1):8000 -v /proc:/host_proc:ro -v /var/run/docker.sock:/var/run/docker.sock:ro netuitive/docker-agent
